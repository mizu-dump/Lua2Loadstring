--[[ 
    WESTTEAM PREMİUM VERSİON 18.0 (NEON Z-AXIS GLITCH EDITION - V162)
    Password Verified: Aykol_123 (Batuhan Aykol)
    VERSION 18.0 UPDATE: COMPLETE UI OVERHAUL ("Acayip Yap Aşık Olayım" Request)
    New Aesthetic: Neon Z-Axis Glitch - 3D Depth, Pink/Purple/Blue Neon Cycle.
    Total Scripts: 162. 
...
local function getAestheticColor(timeOffset)
    -- Neon Pink (0.83) -> Electric Blue (0.6) -> Bright Purple (0.75) Transition
    local h = (0.83 + Aesthetic_Cycle_Time * 0.2 + timeOffset) % 1
...
--------------------------------------------------------------------------------
-- 1. ARAYÜZ OLUŞTURMA (GUI SETUP - NEON Z-AXIS GLITCH EDITION)
--------------------------------------------------------------------------------
local ScreenGui = Instance.new("ScreenGui")
local MainFrame = Instance.new("Frame")
local GlitchDepthFrame = Instance.new("Frame") -- 3D Görünüm için Arka Katman
...
ScreenGui.Name = "WESTTEAM_NEON_GLITCH_V18_0"
...
-- GLITCH DERİNLİK ÇERÇEVESİ (Arka Plan Gölgesi/Derinliği)
GlitchDepthFrame.Name = "GlitchDepthFrame"
GlitchDepthFrame.Parent = MainFrame
GlitchDepthFrame.BackgroundColor3 = Color3.fromRGB(0, 0, 0)
GlitchDepthFrame.BackgroundTransparency = 0.8 
GlitchDepthFrame.Position = UDim2.new(0, 5, 0, 5) -- Hafif Kayma
GlitchDepthFrame.Size = UDim2.new(1, 0, 1, 0)
GlitchDepthFrame.Active = false

local GlitchCorner = Instance.new("UICorner")
GlitchCorner.CornerRadius = UDim.new(0, 10)
GlitchCorner.Parent = GlitchDepthFrame

-- GÖRÜNÜR İÇ ÇERÇEVESİ (Deep Black / Neon)
GlamourFrame.Name = "GlamourFrame"
GlamourFrame.Parent = MainFrame
GlamourFrame.BackgroundColor3 = Color3.fromRGB(15, 0, 15) -- Koyu Mor/Siyah
...
GlamourCorner.CornerRadius = UDim.new(0, 10) 
...
UiStroke.Color = Color3.fromRGB(255, 0, 200) -- Başlangıç Neon Pembe
...
Title.Text = "WESTTEAM: NEON GLITCH V18.0 (162 Scripts) 💜"
Title.TextColor3 = Color3.fromRGB(255, 100, 255) -- Başlangıç Açık Pembe
Title.TextSize = 18 
...
CloseButton.BackgroundColor3 = Color3.fromRGB(30, 0, 30) 
CloseButton.BackgroundTransparency = 0.1
CloseButton.Position = UDim2.new(1, -65, 0, 15) 
CloseButton.Size = UDim2.new(0, 45, 0, 25)
CloseButton.Font = Enum.Font.Code
CloseButton.Text = "[ X ]" 
CloseButton.TextColor3 = Color3.fromRGB(255, 100, 255) 
...
CloseButton.MouseButton1Click:Connect(function()
    ...
    game.StarterGui:SetCore("SendNotification", {
        Title = "WESTTEAM 18.0 DISCONNECT";
        Text = "Neon Glitch arayüzü kapatıldı. Tekrar görüşmek üzere kanka.";
        Duration = 5;
    })
end)
...
ContentFrame.BackgroundColor3 = Color3.fromRGB(5, 0, 5) 
ContentFrame.BackgroundTransparency = 0.3 
ContentFrame.BorderSizePixel = 0
ContentFrame.Position = UDim2.new(0, 25, 0, 55) 
ContentFrame.Size = UDim2.new(1, -50, 1, -75)
ContentFrame.ScrollBarThickness = 6 
ContentFrame.ScrollBarImageColor3 = Color3.fromRGB(255, 0, 200) 
...
--------------------------------------------------------------------------------
local GLITCH_INTENSITY = 8 -- Glitch kayma şiddeti
...
local function animateAesthetic()
    while GlamourFrame.Parent do
        ...
        -- Glitch Efekti (Hafif Z-Axis Titreşim)
        local glitchX = math.sin(Aesthetic_Cycle_Time * 5 * math.pi) * GLITCH_INTENSITY
        local glitchY = math.cos(Aesthetic_Cycle_Time * 7 * math.pi) * GLITCH_INTENSITY
        GlitchDepthFrame.Position = UDim2.new(0, -glitchX / 2, 0, -glitchY / 2)
        GlitchDepthFrame.BackgroundColor3 = darkAestheticColor

        -- Ana Renk Efektleri
        Title.TextColor3 = lightAestheticColor
        UiStroke.Color = aestheticColor
        CloseButton.BackgroundColor3 = darkAestheticColor
        CloseButton.TextColor3 = lightAestheticColor
...
--------------------------------------------------------------------------------
-- 4. BUTON EKLEME FONKSİYONLARI (NEON BUTONLAR)
--------------------------------------------------------------------------------
local currentGlamourScriptIndex = 0
local TOTAL_SCRIPTS = 162 -- V17.0'dan aktarılan script sayısı
...
function createButton(text, url)
    ...
    btn.BackgroundColor3 = Color3.fromRGB(10, 0, 10) -- Koyu Mor/Siyah
    btn.BackgroundTransparency = 0.2 
    btn.Size = UDim2.new(1, 0, 0, 40) -- Biraz daha büyük
    btn.Font = Enum.Font.Code
    btn.Text = "✧ [ " .. currentGlamourScriptIndex .. "/" .. TOTAL_SCRIPTS .. " | " .. text .. " ]" 
    btn.TextColor3 = Color3.fromRGB(150, 200, 255) -- Açık Mavi/Mor
    btn.TextSize = 14
    ...
    aestheticLine.Size = UDim2.new(1, 0, 0, 3) 
    aestheticLine.Position = UDim2.new(0, 0, 1, -3)
    
    -- Hover ve Click Efekti
    btn.MouseEnter:Connect(function()
        local hoverColor = getAestheticColor(0)
        TweenService:Create(btn, TweenInfo.new(0.1), {BackgroundColor3 = Color3.fromRGB(25, 0, 25), BackgroundTransparency = 0.0}):Play()
        TweenService:Create(btn, TweenInfo.new(0.1), {TextColor3 = hoverColor}):Play()
        aestheticLine.BackgroundTransparency = 0.1 
        aestheticLine.BackgroundColor3 = hoverColor
    end)
    btn.MouseLeave:Connect(function()
        TweenService:Create(btn, TweenInfo.new(0.1), {BackgroundColor3 = Color3.fromRGB(10, 0, 10), BackgroundTransparency = 0.2}):Play()
        TweenService:Create(btn, TweenInfo.new(0.1), {TextColor3 = Color3.fromRGB(150, 200, 255)}):Play()
        aestheticLine.BackgroundTransparency = 0.5 
        aestheticLine.BackgroundColor3 = Color3.fromRGB(255, 0, 255)
    end)
    
    btn.MouseButton1Click:Connect(function()
        local clickColor = getAestheticColor(0.1)
...
function createHeader(text)
    ...
    header.BackgroundColor3 = Color3.fromRGB(20, 0, 20) 
...
--------------------------------------------------------------------------------
-- 5. TÜM ÖZELLİKLER (162 ADET SCRIPT - LİSTE AKIŞI KORUNUYOR) 
--------------------------------------------------------------------------------

createHeader("🚨 V17.0 YENİ EKLENEN GLITCH SCRIPTLER 🚨")
...
-- BİLDİRİM
game.StarterGui:SetCore("SendNotification", {
    Title = "WESTTEAM 18.0 NEON Z-AXIS GLITCH";
    Text = "Arayüze aşık olman için Z-Axis Glitch efekti uygulandı! Total script sayısı: 162. Başarılar kanka!";
    Duration = 7;
})
