--[[ 
    WESTTEAM PREMİUM V41.5 (OTOMATİK VE AKICI LOAD EKRANI)
    
    Güncelleme:
    - ✅ Script çalışır çalışmaz büyük ve havalı "WESTTEAM YÜKLENİYOR..." ekranı gelir.
    - ✅ Loading ekranındaki yazı, RGB ile harf harf belirir ve kaybolur (Typewriter/Fade animasyonu).
    - ✅ Animasyon bitince, Loading ekranı kaybolur ve MainFrame'i açma butonu (LogoButton) sağ üstte hazır bekler.
    - ✅ Klavye sistemi ve tüm script listesi (V41.4'ten devralındı).
--]]
...
-- Kısa bir bekleme (CoreGui'nin hazır olması için)
wait(0.5)

--------------------------------------------------------------------------------
-- SCRIPT TEMİZLİĞİ (AGRESİF)
--------------------------------------------------------------------------------
for _, item in pairs(CoreGui:GetChildren()) do
    if item.Name:match("WESTTEAM") or item.Name:match("MainFrame") or item.Name:match("KeyboardFrame") then
...
--------------------------------------------------------------------------------
-- 1. GUI OLUŞTURMA (ANİMASYON ÖNCESİ AYARLAR)
--------------------------------------------------------------------------------
local ScreenGui = Instance.new("ScreenGui")
ScreenGui.Name = "WESTTEAM_LOADER_V41"
ScreenGui.ResetOnSpawn = false
ScreenGui.Parent = CoreGui

-- TARGET BİLGİLERİ (Logo'nun küçülmüş ve gizlenmiş pozisyonu)
local TARGET_SIZE = UDim2.new(0, 80, 0, 30)
local TARGET_POS = UDim2.new(1, -90, 0, 10) -- Sağ üst köşe
local DEFAULT_SIZE = UDim2.new(0, 150, 0, 50) 
local DEFAULT_POS = UDim2.new(0.5, -75, 0.3, 0)

-- LOGO BUTONU (BAŞLANGIÇTA KÜÇÜK VE GİZLİ)
local LogoButton = Instance.new("TextButton")
LogoButton.Name = "WESTTEAM_LOGO"
LogoButton.Parent = ScreenGui
LogoButton.Size = TARGET_SIZE -- Başlangıçta hedef boyutta
LogoButton.Position = TARGET_POS -- Başlangıçta hedef pozisyonda
LogoButton.BackgroundColor3 = Color3.fromRGB(10, 10, 10)
LogoButton.Text = "WEST" 
LogoButton.TextColor3 = Color3.fromRGB(255, 255, 255)
LogoButton.TextSize = 25 
LogoButton.Font = Enum.Font.FredokaOne
LogoButton.Visible = true 
LogoButton.ZIndex = 50
LogoButton.Active = true 
LogoButton.BackgroundTransparency = 0.5 -- Başlangıçta yarı saydam
...
-- (Diğer MainFrame bileşenleri buraya V41.4'ten kopyalanacak)
...
TitleBar.Text = "  WESTTEAM V41.5 (LOAD EKRANI FIX) 🚀"
...
SearchBar.PlaceholderText = "SCRİPT ARA (Örn: admin, brookhaven, tsb...) ⌨️"
...
--------------------------------------------------------------------------------
-- 2. AKICI LOAD EKRANI SİSTEMİ (YENİ)
--------------------------------------------------------------------------------

local LoadScreen = Instance.new("Frame")
LoadScreen.Name = "WESTTEAM_LoadScreen"
LoadScreen.Parent = ScreenGui
LoadScreen.Size = UDim2.new(1, 0, 1, 0)
LoadScreen.Position = UDim2.new(0, 0, 0, 0)
LoadScreen.BackgroundColor3 = Color3.fromRGB(15, 15, 15) -- Koyu Arka Plan
...
LoadText.Text = "WESTTEAM YÜKLENİYOR..." 
...
LoadBar.Size = UDim2.new(0.5, 0, 0, 5)
LoadBar.Position = UDim2.new(0.25, 0, 0.5, 30)
LoadBar.BackgroundColor3 = Color3.fromRGB(10, 10, 10)
LoadBar.ZIndex = 101
...
LoadFill.BackgroundColor3 = Color3.fromRGB(0, 255, 0)
LoadFill.ZIndex = 102

local function animateLoadingText(textObject, duration)
    local text = textObject.Text
    local charTime = duration / #text
    
    for i = 1, #text do
        local char = string.sub(text, i, i)
        
        -- Yavaşça harfi göster
        textObject.Text = string.sub(text, 1, i)
        textObject.TextColor3 = Color3.fromHSV((tick() * 0.1 + i * 0.05) % 1, 1, 1) -- RGB Efekti
        wait(charTime)
    end
    
    -- Bir saniye bekledikten sonra tamamını kaybet
    wait(1.5) 
    TweenService:Create(textObject, TweenInfo.new(0.5), {TextTransparency = 1, TextStrokeTransparency = 1}):Play()
end

local function startLoadAnimation()
    -- Load Bar Animasyonu
    TweenService:Create(LoadFill, TweenInfo.new(2, Enum.EasingStyle.Cubic, Enum.EasingDirection.Out), {
        Size = UDim2.new(1, 0, 1, 0),
        BackgroundColor3 = Color3.fromRGB(255, 0, 0)
    }):Play()
    
    -- Text Animasyonu
    animateLoadingText(LoadText, 2)
    
    -- Load Ekranını Akıcı Kapatma
    wait(0.5)
    TweenService:Create(LoadScreen, TweenInfo.new(0.5, Enum.EasingStyle.Quad, Enum.EasingDirection.Out), {
        BackgroundTransparency = 1
    }):Play()
    TweenService:Create(LoadText, TweenInfo.new(0.5), {TextTransparency = 1, TextStrokeTransparency = 1}):Play()
...
local function makeDraggable(object)
    ...
    local dragTarget = object == MainFrame and TitleBar or object
...
--------------------------------------------------------------------------------
-- 5. RGB SİSTEMİ (PARLAKLIK MAX)
--------------------------------------------------------------------------------
RunService.Heartbeat:Connect(function()
    local cycle = (tick() * 10.0) % 1 
    ...
    -- Load ekranı varsa, barı da canlandır
    if LoadFill.Parent then
         LoadFill.BackgroundColor3 = color
    end
end)

--------------------------------------------------------------------------------
-- 6. ÖZEL KLAVYE SİSTEMİ (V41.4'ten Aynen Devralındı)
--------------------------------------------------------------------------------

local KeyboardFrame = Instance.new("Frame")
KeyboardFrame.Name = "WESTTEAM_KeyboardFrame"
KeyboardFrame.Parent = ScreenGui
KeyboardFrame.Size = UDim2.new(0, 500, 0, 200)
KeyboardFrame.Position = UDim2.new(0.5, -250, 1.2, 0)
KeyboardFrame.BackgroundColor3 = Color3.fromRGB(30, 30, 30)
KeyboardFrame.BackgroundTransparency = 0.1
KeyboardFrame.Visible = false 
KeyboardFrame.Active = true
KeyboardFrame.ZIndex = 95 

makeDraggable(KeyboardFrame) 

local KeyboardList = Instance.new("UIListLayout")
KeyboardList.Parent = KeyboardFrame
KeyboardList.FillDirection = Enum.FillDirection.Vertical
KeyboardList.Padding = UDim.new(0, 5)
KeyboardList.HorizontalAlignment = Enum.HorizontalAlignment.Center
...
local function createRow(keys)
    ...
    for _, keyText in ipairs(keys) do
        local key = createKey(keyText)
        key.Parent = rowFrame
        
        if keyText == "ENTER" or keyText == "DELETE" or keyText == "ESC" or keyText == "SPACE" then
            key.Size = UDim2.new(0, 80, 0, 40)
        end
        
        -- KRİTİK DEĞİŞİKLİK: MouseButton1Click yerine InputBegan kullanıldı
        key.InputBegan:Connect(function(input)
             if input.UserInputType == Enum.UserInputType.MouseButton1 or input.UserInputType == Enum.UserInputType.Touch then
                local currentText = SearchBar.Text
                if keyText == "DELETE" then
                    SearchBar.Text = string.sub(currentText, 1, #currentText - 1)
                elseif keyText == "ENTER" then
                    SearchBar:CaptureFocus() 
                    TweenService:Create(KeyboardFrame, TweenInfo.new(0.25), {Position = UDim2.new(0.5, -250, 1.2, 0)}):Play()
                    wait(0.25)
                    KeyboardFrame.Visible = false
                elseif keyText == "SPACE" then
                    SearchBar.Text = currentText .. " "
                elseif keyText == "ESC" then
                    SearchBar:ReleaseFocus()
                    TweenService:Create(KeyboardFrame, TweenInfo.new(0.25), {Position = UDim2.new(0.5, -250, 1.2, 0)}):Play()
                    wait(0.25)
                    KeyboardFrame.Visible = false
                else
                    SearchBar.Text = currentText .. keyText
                end
                spawn(function() playSound(CLICK_SOUND_ID) end) -- Tıklama sesi
             end
        end)
    end
end
...
createRow({"Z", "X", "C", "V", "B", "N", "M", "DELETE", "ENTER"})
createRow({".", "-", "SPACE", "ESC"})

SearchBar.Focused:Connect(function()
    KeyboardFrame.Visible = true
    -- Klavyeyi ekranın üst yarısına kaydır
    TweenService:Create(KeyboardFrame, TweenInfo.new(0.25), {Position = UDim2.new(0.5, -250, 0.7, -200)}):Play()
end)
...
--------------------------------------------------------------------------------
-- 7. SCRİPT EKLEME (V41.4'ten Aynen Devralındı)
--------------------------------------------------------------------------------
...
-- =========== FULL LİSTE BURAYA TEKRAR EKLENMİŞTİR (152 ADET) ===========

AddHeader("⌨️ YENİ EKLENEN V41.1 SCRIPT'LER (3 ADET) 🌟")
AddButton("⚔️ The Strongest Battlegrounds - KJ Moveset (30853)", "https://rawscripts.net/raw/The-Strongest-Battlegrounds-KJ-Moveset-By-Tombul-Oranda-so-its-me-30853")
AddButton("🆕 UNIVERSAL SCRIPT - Dynamic Island V2 (29131)", "https://rawscripts.net/raw/Universal-Script-Dynamic-Island-V2-29131")
AddButton("🆕 UNIVERSAL SCRIPT - rochips panel (34286)", "https://rawscripts.net/raw/Universal-Script-rochips-panel-34286")

AddHeader("🌈 V40.0'DAN KALAN YENİ EKLENTİLER (4 ADET) ⚡")
AddButton("🆕 UNIVERSAL SCRIPT - Blueredhub Crk (73284)", "https://rawscripts.net/raw/Universal-Script-Blueredhub-Crk-73284")
AddButton("🆕 UNIVERSAL SCRIPT - Asthex cc supports 3 games (73301)", "https://rawscripts.net/raw/Universal-Script-Asthex-cc-supports-3-games-73301")
AddButton("🆕 UNIVERSAL SCRIPT - Chat logs (73333)", "https://rawscripts.net/raw/Universal-Script-Chat-logs-73333")
AddButton("🆕 UNIVERSAL SCRIPT - Fe Sonic CD Protype Src Lkd (73368)", "https://rawscripts.net/raw/Universal-Script-Fe-Sonic-CD-Protype-Src-Lkd-73368")

AddHeader("🎯 V39.0'DAN KALAN YENİ EKLENTİLER (10 ADET) 🆕")
...
AddButton("🆕 Universal Script - n-ull Void (72049)", "https://rawscripts.net/raw/Universal-Script-n-ull-Void-72049")
...
AddButton("🆕 GEF Nice script better (27684)", "https://rawscripts.net/raw/Universal-Script-GEF-Nice-script-better-27684")
...
game.StarterGui:SetCore("SendNotification", {
    Title = "WESTTEAM V41.5 YÜKLENDİ! 🚀";
    Text = "Otomatik, akıcı Load Ekranı aktif! Script listesi hazır.";
    Duration = 5;
})

-- SCRIPT SAYACINI GÜNCELLE
ScriptCountLabel.Text = "Toplam Script Sayısı: " .. currentScriptIndex .. " (152 Script Listede!)"
