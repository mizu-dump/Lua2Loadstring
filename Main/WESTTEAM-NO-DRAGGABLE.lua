--[[ 
    WESTTEAM PREMİUM VERSİON 32.0 (MAX SEARCH BOOST EDITION)
    Password Verified: Aykol_123 (Batuhan Aykol)
    VERSION 32.0 CRITICAL FIXES & ENHANCEMENTS: 
    - CRITICAL: Search Functionality upgraded to MAX LEVEL. Now supports:
        1. AlphaFilter (Starts With) - High priority matches.
        2. SubString Search (Contains) - Matches anywhere in the script name.
    - FEATURE: 17 new powerful scripts added to the list (Tubers93, C00lkidd, Slap Battles L-hub, etc.).
    - MAINTAINED: Glamour Drag (Slippery UI) and Full Script List are preserved.
...
Title.Text = "WESTTEAM: OMEGA V32.0 ✨ MAX SEARCH"
...
CloseButton.MouseButton1Click:Connect(function()
    ...
    game.StarterGui:SetCore("SendNotification", {
        Title = "WESTTEAM 32.0 DISCONNECT";
...
SearchBar.PlaceholderText = "MAX SEARCH (Örn: admin, fly, 99 nights)"
...
-- MAX SEARCH (AlphaFilter + SubString Search)
local function filterScripts(searchText)
    ...
    if searchLength == 0 then
        ...
        -- Adım 1: Scriptleri filtrele (Başlangıç Eşleşmesi VEYA İçerik Eşleşmesi)
        ...
        for i, scriptData in ipairs(allScripts) do
            ...
            if not isHeader then 
                ...
                -- 1. Alpha Filter (Başlangıç Eşleşmesi)
                local scriptPrefix = string.sub(lowerScriptText, 1, searchLength)
                
                if scriptPrefix == lowerSearchText then
                    visible = true
                
                -- 2. SubString Search (İçerik Eşleşmesi - Metnin herhangi bir yerinde)
                elseif string.find(lowerScriptText, lowerSearchText, 1, true) then 
                    visible = true
                end
                ...
                if visible and scriptData.headerData then
                    ...
                    visibleHeaders[scriptData] = true -- Burada script'in kendisi yerine header'ı işaretlemeliyiz.
                    visibleHeaders[scriptData.headerData] = true -- Ait olduğu header'ı işaretle
                end
            end
        end
...
local TOTAL_SCRIPTS = 201 -- 17 yeni script eklendi
...
--------------------------------------------------------------------------------
-- 5. TÜM ÖZELLİKLER (201+ ADET SCRIPT - GÜNCELLENMİŞ TAM LİSTE)
--------------------------------------------------------------------------------

createHeader("🚨 KRİTİK EKLENTİLER (17 YENİ SCRIPT EKLENDİ) 🆕")
...
createButton("🆕 SLAP BATTLES - L-HUB (68923)", "https://rawscripts.net/raw/UPDATE-Slap-Battles-L-hub-68923")
...
LoadText.Text = "WESTTEAM OMEGA V32.0 READY"
...
-- BİLDİRİM
game.StarterGui:SetCore("SendNotification", {
    Title = "WESTTEAM 32.0 HAZIR (MAX SEARCH AKTİF)";
    Text = "Kanka, 17 yeni script eklendi! Arama sistemi artık metnin neresinde olursa olsun eşleşmeleri bulacak. Max seviye arama aktif!";
    Duration = 7;
})
