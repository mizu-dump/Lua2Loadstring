...
SpinnyThingyWeld=weld(script.Armor.MiniGun.SpinPos,script.Armor.MiniGun.SpinPos,SpinnyThingy,cf(0,0,0))
--------------------------------------------Hand
...
if not Xgui then
    warn("Xgui dont Exist")
    return
end

local FAT = Xgui:FindFirstChild("FAT")
if not FAT then
    warn("frame dont Exist")
    return
end

local viewport = FAT:FindFirstChildOfClass("ViewportFrame") or FAT:FindFirstChild("ViewportFrame")
if not viewport then
    warn("ViewportFrame dont Exist")
    return
end
...
local function keyToString(input)
    -- se for TextInput (teclas de letra) prefere .KeyCode.Name, depois pega primeira letra
    local name = tostring(input.KeyCode) -- exemplo: "Enum.KeyCode.F"
    name = name:gsub("Enum.KeyCode.", "") -- fica "F"
    name = name:lower()
    return name
end

UserInputService.InputBegan:Connect(function(input, gameProcessed)
    if gameProcessed then return end -- não intercepta digitação em GUIs
    if input.UserInputType ~= Enum.UserInputType.Keyboard then return end

    local k = keyToString(input)

    if type(KeyDownF) == "function" then
        local ok, err = pcall(function() KeyDownF(k) end)
        if not ok then
            warn("Erro em KeyDownF:", err)
        end
    else

        if type(KeyDown) == "function" then
            local ok, err = pcall(function() KeyDown(k) end)
            if not ok then warn("Erro em KeyDown:", err) end
        end
    end
end)
...
UserInputService.InputEnded:Connect(function(input, gameProcessed)
    ...
    if type(KeyUpF) == "function" then
        ...
        if not ok then warn("Erro em KeyUpF:", err) end
    else
        if type(KeyUp) == "function" then
            local ok, err = pcall(function() KeyUp(k) end)
            if not ok then warn("Erro em KeyUp:", err) end
        end
    end
end)