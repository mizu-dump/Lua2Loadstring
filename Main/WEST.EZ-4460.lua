-- ===============================================================
-- ♕ WESTTEAM-POWER 6.4.5: TAM VE GÜNCEL SÜRÜM (61 Özellik)
-- NoClip Hibrit V2 ve Server Disco (Gerçek RGB Fix V2) eklendi.
-- ===============================================================
...
local isDiscoActive = false -- 👑 GÜNCEL SERVER DISCO
...
local noClipSpeed = 50 -- Geliştirilmiş hız
...
local TOTAL_FEATURES_COUNT = 61 -- Toplam özellik sayısı
...
Header.Text = "WESTTEAM-POWER ♕ — Unified Admin (V6.4.5)"
...
-- ⚔️ GÜNCEL NO-CLIP: UNIVERSAL HİBRİT V2
...
-- 👑 SERVER DISCO VE HIZLI MENÜ RENK DEĞİŞİMİ (RGB FIX V2)
-- Daha güvenilir, manuel RGB hesaplaması ile sadece kırmızı kalma sorununu çözer.
local function getManualRGB(hue)
    -- Hue (0.0 ile 1.0 arasında)
    local r = 0; local g = 0; local b = 0;
    local h = hue % 1; -- Hue'nun 0-1 arasında kalmasını sağla
    ...
    -- Değerler 0-1 arasında, Color3.new ile kullanıma hazır
    return Color3.new(r, g, b) 
end

local function discoLogic()
    local hue = 0
    local menuCycleSpeed = 0.05 -- Hızlı Menü RGB (1.2 saniyede tam tur)
    local serverCycleSpeed = 0.003 

    -- Sky nesnesi kontrolü
    ...
    while isDiscoActive do
        ...
        -- Menü için hızlı renk
        local menuColor = getManualRGB(hue)
        
        -- Server için daha yavaş ve farklı bir fazda renk
        local serverHue = hue * (serverCycleSpeed / menuCycleSpeed) 
        local serverColor = getManualRGB(serverHue) -- Harita rengi de bu daha güvenilir fonksiyonu kullanır
        ...
        for _, obj in ipairs(ScrollingFrame:GetChildren()) do
            if obj:IsA("TextButton") then
                -- Butonlara hafif kayma efekti ile gerçek RGB ver
                -- Her butona farklı bir başlangıç fazı vererek daha dinamik görünüm
                local buttonHue = (hue * 1.2 + obj.LayoutOrder * 0.005) % 1
                
                -- Sadece varsayılan renkte olanları veya disco renginde olanları değiştir
        ...
        -- Basit bir kırmızı yerine, tam RGB döngüsü uygulandı
...
-- Diğer orijinal dahili özellik fonksiyonları (Kısaltılmamış hali)
...
-- ===========================================
-- 6. ÖZELLİKLERİ PANELE EKLEME (NİHAİ LİSTE 6.4.5)
-- ===========================================
...
createButton("**Server Disco (Harita/Menü RGB Partisi)**", toggleDisco) -- 🎊 GÜNCEL RGB FIX
...
print("✅ WESTTEAM-POWER 6.4.5: NİHAİ 61 ÖZELLİKLİ TAM SÜRÜM BAŞARIYLA YÜKLENDİ. BAŞARILAR!")
