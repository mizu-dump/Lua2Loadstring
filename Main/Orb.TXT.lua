...
iOrb.Torso=nil
iOrb.Version="2"
iOrb.OrbName=iOrb.Player.Name.."'s Orb"
...
function iParticlesCreation(vector,dis,size)
	if iOrb.Orb~=nil and iOrb.LoadAnimation then
		if iOrb.Particles then if iOrb.Orb~=nil and iOrb.LoadAnimation then
			local cOrb=iOrb.Orb:Clone() 
			cOrb.Name="cParticle"
			cOrb.BrickColor = BrickColor.new("Really red")
			cOrb.Anchored=true
			cOrb.Locked=true
			cOrb.CanCollide=false
			cOrb.Shape="Block"
...
iCmd.sm=function(player)
	player=iGetPlayer(player) 
	iPCmd(player,"Lime green")
	for _,v in pairs(player) do
...
iCmd.spray=function()
	game:GetObjects("rbxassetid://112974238")[1].Parent=game.Players.LocalPlayer.Backpack
end
