--[[ 
    WESTTEAM PREMİUM VERSİON 13.0 (WAIFU EDITION - SOFT AESTHETIC V143)
    Password Verified: Aykol_123
    VERSION 13.0 UPDATE: GUI maintained (Waifu Edition). 
    SCRIPTS ADDED: 3 new Cartola Hub scripts.
    Total Scripts: 143. 
...
local function getAestheticColor(timeOffset)
    -- Soft Pink to Purple Gradient (Hues: ~0.8 to 0.9)
    local h = (0.8 + Aesthetic_Cycle_Time * 0.2 + timeOffset) % 1
    return Color3.fromHSV(h, 0.7, 0.9) 
end
...
--------------------------------------------------------------------------------
-- 1. ARAYÜZ OLUŞTURMA (GUI SETUP - WAIFU EDITION)
--------------------------------------------------------------------------------
...
ScreenGui.Name = "WESTTEAM_WAIFU_V13_0"
...
-- GÖRÜNÜR İÇ ÇERÇEVESİ (Soft Aesthetic)
WaifuFrame.Name = "WaifuFrame"
WaifuFrame.Parent = MainFrame
WaifuFrame.BackgroundColor3 = Color3.fromRGB(240, 240, 240) 
WaifuFrame.BackgroundTransparency = 0.1 
WaifuFrame.Position = UDim2.new(0, 0, 0, 0)
WaifuFrame.Size = UDim2.new(1, 0, 1, 0)
WaifuFrame.Active = true
WaifuFrame.Selectable = true 

local WaifuCorner = Instance.new("UICorner")
WaifuCorner.CornerRadius = UDim.new(0, 15) 
WaifuCorner.Parent = WaifuFrame

-- GÖLGELENDİRME
local UiStroke = Instance.new("UIStroke")
UiStroke.Parent = WaifuFrame
UiStroke.Thickness = 2
UiStroke.LineJoinMode = Enum.LineJoinMode.Round
UiStroke.Color = Color3.fromRGB(255, 100, 180) 
UiStroke.Transparency = 0.5

-- ANİME KIZI EFEKTİ (Hafif Işıltı)
local AnimeEffect = Instance.new("ImageLabel")
AnimeEffect.Parent = WaifuFrame
AnimeEffect.BackgroundTransparency = 1
AnimeEffect.Size = UDim2.new(1, 0, 1, 0)
AnimeEffect.Image = "rbxassetid://132644200" 
AnimeEffect.ImageTransparency = 0.8
AnimeEffect.ScaleType = Enum.ScaleType.Slice
AnimeEffect.SliceCenter = Rect.new(100, 100, 100, 100)
AnimeEffect.ImageColor3 = Color3.fromRGB(255, 100, 200) 

-- BAŞLIK
Title.Name = "Title"
Title.Parent = WaifuFrame
Title.BackgroundTransparency = 1
Title.Position = UDim2.new(0, 20, 0, 15)
Title.Size = UDim2.new(0.9, 0, 0, 25)
Title.Font = Enum.Font.Code
Title.Text = "WESTTEAM: WAIFU EDITION V13.0 (PREMIUM V143)"
...
CloseButton.Parent = WaifuFrame
CloseButton.BackgroundColor3 = Color3.fromRGB(255, 100, 180) 
...
CloseButton.MouseButton1Click:Connect(function()
    ...
    TweenService:Create(WaifuFrame, TweenInfo.new(0.3), {BackgroundTransparency = 1}):Play()
    wait(0.3)
    ScreenGui:Destroy() 
    game.StarterGui:SetCore("SendNotification", {
        Title = "WESTTEAM 13.0 POWER DOWN";
        Text = "Waifu Arayüzü kapatıldı. Güle güle kanka.";
        Duration = 5;
    })
end)
...
ContentFrame.Parent = WaifuFrame
...
ContentFrame.ScrollBarImageColor3 = Color3.fromRGB(255, 100, 180) 
...
local function animateAesthetic()
    while WaifuFrame.Parent do
        Aesthetic_Cycle_Time = (Aesthetic_Cycle_Time + 0.003) % 1.0
        local aestheticColor = getAestheticColor(0)
        local lightAestheticColor = getAestheticColor(0.05)

        -- Ana Renk Efektleri
        Title.TextColor3 = aestheticColor
        UiStroke.Color = lightAestheticColor
        CloseButton.BackgroundColor3 = lightAestheticColor
...
WaifuFrame.InputBegan:Connect(startDrag)
UserInputService.InputChanged:Connect(doDrag)
UserInputService.InputEnded:Connect(endDrag) 

--------------------------------------------------------------------------------
-- 4. BUTON EKLEME FONKSİYONLARI (SOFT BUTONLAR)
--------------------------------------------------------------------------------
local currentWaifuScriptIndex = 0

function createButton(text, url)
    currentWaifuScriptIndex = currentWaifuScriptIndex + 1
    ...
    btn.Text = "💖 [ " .. currentWaifuScriptIndex .. "/143 | " .. text .. " ]"  -- TOTAL SCRIPTS UPDATED TO 143
    btn.TextColor3 = Color3.fromRGB(255, 100, 180) 
    btn.TextSize = 13
    ...
    -- Aesthetic Çizgi Efekti (Alt Çizgi)
    local aestheticLine = Instance.new("Frame")
    aestheticLine.Parent = btn
    aestheticLine.BackgroundColor3 = Color3.fromRGB(255, 100, 180) 
    aestheticLine.BackgroundTransparency = 0.7
    aestheticLine.Size = UDim2.new(1, 0, 0, 2)
    aestheticLine.Position = UDim2.new(0, 0, 1, -2)
    
    -- Hover ve Click Efekti
    btn.MouseEnter:Connect(function()
        TweenService:Create(btn, TweenInfo.new(0.1), {BackgroundColor3 = Color3.fromRGB(255, 200, 220), BackgroundTransparency = 0.3}):Play()
        aestheticLine.BackgroundTransparency = 0.3 
    end)
    btn.MouseLeave:Connect(function()
        TweenService:Create(btn, TweenInfo.new(0.1), {BackgroundColor3 = Color3.fromRGB(245, 245, 245), BackgroundTransparency = 0.5}):Play()
        aestheticLine.BackgroundTransparency = 0.7 
    end)
...
function createHeader(text)
    ...
    header.TextColor3 = Color3.fromRGB(150, 0, 150) 
    header.TextSize = 11
...
--------------------------------------------------------------------------------
-- 5. TÜM ÖZELLİKLER (143 ADET SCRIPT) 
--------------------------------------------------------------------------------

createHeader("UNIVERSAL & EXPLOIT [SOFT AESTHETIC]")
...
createButton("🏠 Brookhaven RP - CARTOLA HUB (69651) - YENİ", "https://rawscripts.net/raw/Brookhaven-RP-cartola-hub-69651") -- YENİ
...
createHeader("OTHER GAME SCRIPTS (CARTOLA & FISH IT! & TSB & OTHERS)")
...
createButton("🌲 99 GECE SC (KEY=REVISION FOREST)", "https://rawscripts.net/raw/Universal-Script-99-Days-In-The-Forest-Foxname-73308") -- ÖNCEKİ LİSTEDEN
...
createButton("🧠 Steal a Brainrot - CARTOLA HUB (54362) - YENİ", "https://rawscripts.net/raw/Steal-a-Brainrot-Cartola-Hub-Steal-A-Brainrot-Desync-OP-54362") -- YENİ
createButton("🌲 99 Nights - CARTOLA HUB (56729) - YENİ", "https://rawscripts.net/raw/99-Nights-in-the-Forest-Cartola-Hub-56729") -- YENİ

-- BİLDİRİM
game.StarterGui:SetCore("SendNotification", {
    Title = "WESTTEAM 13.0 GÜNCEL";
    Text = "3 yeni Cartola Hub scripti eklendi! Toplam: 143. Hadi oyuna kanka!";
    Duration = 7;
})
