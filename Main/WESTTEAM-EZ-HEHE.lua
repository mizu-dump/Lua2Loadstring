-- ===============================================================
-- ♕ WESTTEAM-POWER 6.4.9: TAM VE ÇALIŞIR SÜRÜM (69 Özellik)
-- ** GÜNCELLEME: Brainrot Evolution URL'si Düzeltildi (Yeni Kaynak) **
-- ** HATA DÜZELTME: Syntax Error (Eksik 'end') Giderildi. **
-- ===============================================================
...
-- ===========================================
-- 1. ÖZELLİK DURUMLARI (FLAG'LER) - (TOPLAM 69)
-- ===========================================
...
local isTpToolLoaded = false
local isAdminCmdBarLoaded = false      
local isUniversalTornadoLoaded = false  
local isUniversalSCLoaded = false       
...
local isDeadlyScriptsHubLoaded = false 
local isBO111ClanExecutorLoaded = false 
local isPolariaExploderLoaded = false  

-- Bağlantılar, Değişkenler, GUI Ayarları
...
local ANTI_FLING_COLOR = Color3.fromRGB(255, 100, 100)
local BO111_COLOR = Color3.fromRGB(100, 100, 255) 
local POLARIA_COLOR = Color3.fromRGB(255, 0, 150) 
local TPTOOL_COLOR = Color3.fromRGB(0, 255, 100) 
local ADMIN_CMD_COLOR = Color3.fromRGB(255, 255, 0) 
local TORNADO_SC_COLOR = Color3.fromRGB(138, 43, 226)  
local UNIVERSAL_SC_COLOR = Color3.fromRGB(0, 255, 150)  
local BRAINROT_COLOR = Color3.fromRGB(85, 0, 170) 
...
-- ===========================================
-- 2. GUI YAPISI VE HAREKETİ (Kısaltıldı)
-- ===========================================
...
Header.Text = "WESTTEAM-POWER ♕ — Unified Admin (V6.4.9)"
...
-- ===========================================
-- 3. ÖZELLİK FONKSİYONLARI (DAHİLİ 21 ADET) - GÜNCEL KISIMLAR
-- ===========================================
...
-- 1. DAHİLİ ÖZELLİKLER (21 Adet)
local toggleTpTool = createRerunToggle("isTpToolLoaded", "https://rawscripts.net/raw/Universal-Script-TpTool-Teleport-Tool-47255", "**TP TOOL.S (Teleport)**", TPTOOL_COLOR)
local toggleAntiAfk = function(button) 
    isAntiAfkActive = not isAntiAfkActive; 
    local Humanoid = LocalPlayer.Character and LocalPlayer.Character:FindFirstChildOfClass("Humanoid"); 
    if isAntiAfkActive then 
        coroutine.wrap(function() 
            while isAntiAfkActive and Humanoid do 
...
local universalNoClipLogic = function() 
    ...
    if noClipHumanoid:GetState() ~= Enum.HumanoidStateType.PlatformStanding then 
    ...
    noClipHRP.Velocity = Vector3.new(0,0,0)
    noClipHRP.RotationalVelocity = Vector3.new(0,0,0)
end
local toggleNoClip = function(button) 
    ...
    if not noClipHRP or not noClipHumanoid then warn("HRP veya Humanoid bulunamadi."); isNoClipActive = false; button.Text = "**NoClip (Hata: Parça Eksik)**"; button.BackgroundColor3 = BUTTON_COLOR; return end 
    if isNoClipActive then 
        ...
        bodyVelocity.Parent = noClipHRP
        noClipLoop = RunService.Heartbeat:Connect(universalNoClipLogic)
        button.Text = "**NoClip (Aktif - UNIVERSAL HİBRİT V2)**"; 
        button.BackgroundColor3 = ACCENT_COLOR 
    else 
        if noClipLoop then noClipLoop:Disconnect(); noClipLoop = nil end
        if bodyVelocity and bodyVelocity.Parent then bodyVelocity:Destroy(); bodyVelocity = nil end 
        if noClipHRP and noClipHRP.Parent then 
            noClipHRP.CanCollide = true; 
            noClipHRP.Massless = false; 
            noClipHRP.Velocity = Vector3.new(0, 0, 0) 
        end
        if noClipHumanoid then 
            noClipHumanoid.WalkSpeed = originalNoClipWalkSpeed; 
            noClipHumanoid:ChangeState(Enum.HumanoidStateType.Running) 
        end
        button.Text = "**NoClip (Universal Hibrit)**"; 
        button.BackgroundColor3 = BUTTON_COLOR 
    end 
end
local getManualRGB = function(hue) 
...
local discoLoop = function() 
    local hue = 0
    local menuCycleSpeed = 0.05 
    local serverCycleSpeed = 0.003 
    if not Lighting:FindFirstChildOfClass("Sky") then
        local newSky = Instance.new("Sky"); newSky.Parent = Lighting
    end
    while isDiscoActive do
        hue = hue + menuCycleSpeed 
        local menuColor = getManualRGB(hue)
        local serverHue = hue * (serverCycleSpeed / menuCycleSpeed) 
        local serverColor = getManualRGB(serverHue) 
        ...
        for _, obj in ipairs(ScrollingFrame:GetChildren()) do
            if obj:IsA("TextButton") then
                local buttonHue = (hue * 1.2 + obj.LayoutOrder * 0.005) % 1
                if obj.BackgroundColor3 ~= ACCENT_COLOR and obj.BackgroundColor3 ~= Color3.fromRGB(255, 85, 0) then
                     obj.BackgroundColor3 = getManualRGB(buttonHue)
                end
            end
        end
...
local toggleDisco = function(button) 
    ...
    if isDiscoActive then
        ...
        coroutine.wrap(discoLoop)()
        button.Text = "**SERVER DISCO (AKTİF - TAM RGB)**"
...
local handleJumpRequest = function() 
    local Humanoid = LocalPlayer.Character and LocalPlayer.Character:FindFirstChildOfClass("Humanoid"); 
    if isInfiniteJumpActive and Humanoid and Humanoid.Health > 0 then 
        Humanoid:ChangeState(Enum.HumanoidStateType.Jumping) 
    end 
end
local toggleInfiniteJump = function(button) 
...
local antiFreezeLoop = function() 
    while isAntiFreezeActive and LocalPlayer.Character do 
        local Humanoid = LocalPlayer.Character:FindFirstChildOfClass("Humanoid"); 
        if Humanoid and Humanoid.Health > 0 then 
            local currentState = Humanoid:GetState(); 
            if currentState == Enum.HumanoidStateType.None or currentState == Enum.HumanoidStateType.Seated or currentState == Enum.HumanoidStateType.PlatformStanding or currentState == Enum.HumanoidStateType.Physics then 
...
local toggleAntiFreeze = function(button) 
...
local aimBotLoop = function() 
    ...
    if target and target:FindFirstChild(AIM_PART) then 
        ...
        Workspace.CurrentCamera.CFrame = CFrame.lookAt(Workspace.CurrentCamera.CFrame.Position, headPosition) 
    end 
end
local toggleAimBot = function(button) 
...
local createPlayerBoxESP = function(player) 
    ...
    for i=1, 4 do 
        ...
        edge.Name = "Edge" .. i; 
        edge.BackgroundColor3 = Color3.fromRGB(0, 255, 255); 
    ...
    for _, player in ipairs(Players:GetPlayers()) do 
        if player ~= LocalPlayer and player.Character then 
            ...
            if HRP and Head then 
                ...
                local billboard = espDrawings[player] and espDrawings[player][1]; 
                if not boxFrame or not billboard then continue end; 
                ...
                if upperVis and lowerVis then 
                    ...
                    boxData[1].Position = UDim2.new(0, topLeft.X, 0, topLeft.Y); 
                    boxData[1].Size = UDim2.new(0, lineThickness, 0, height); 
                    boxData[2].Position = UDim2.new(0, topLeft.X + width - lineThickness, 0, topLeft.Y); 
                    boxData[2].Size = UDim2.new(0, lineThickness, 0, height); 
                    boxData[3].Position = UDim2.new(0, topLeft.X, 0, topLeft.Y); 
                    boxData[3].Size = UDim2.new(0, width, 0, lineThickness); 
                    boxData[4].Position = UDim2.new(0, topLeft.X, 0, topLeft.Y + height - lineThickness); 
                    boxData[4].Size = UDim2.new(0, width, 0, lineThickness); 
                ...
                if espDrawings[player] then 
                    ...
                    espDrawings[player] = nil -- Hata düzeltme: Yanlış sıfırlama
                end 
            end 
        end 
    end 
end
local toggleESP = function(button) 
    ...
    if isEspActive then 
        ...
        espDrawings = {}; -- Hata düzeltme: Doğru sıfırlama
...
local freezeAllActivation = function(shouldFreeze) 
    for _, player in ipairs(Players:GetPlayers()) do 
        if player ~= LocalPlayer then 
            local char = player.Character; 
            if char then 
...
local toggleFreezeAll = function(button) 
...
local toggleFloat = function(button) 
...
local animationGlitchLoop = function() 
    ...
    local Anim = character:FindFirstChildOfClass("Animate") -- Animate script'i 
    if Anim then 
        Anim:Destroy() 
    end
end
local toggleAnimationGlitch = function(button) 
...
local killAuraLoop = function() 
    local myChar = LocalPlayer.Character; 
    local myHRP = myChar and myChar:FindFirstChild("HumanoidRootPart"); 
    local root = myChar and myChar:FindFirstChild("HumanoidRootPart"); 
    if not myChar or not myHRP or not root then return end; 
    for _, player in ipairs(Players:GetPlayers()) do 
        if player ~= LocalPlayer and player.Character then 
...
local toggleKillAura = function(button) 
...
local setWalkSpeed = function(speed) 
    local Humanoid = LocalPlayer.Character and LocalPlayer.Character:FindFirstChildOfClass("Humanoid"); 
    if Humanoid then 
        Humanoid.WalkSpeed = speed 
    end 
end
local toggleWalkSpeed100 = function(button) 
...
local toggleFullBright = function(button) 
...
local toggleFOV = function(button) 
...
local toggleServerHop = function(button) 
    if isServerHopRunning then return end; 
    isServerHopRunning = true; 
    button.Text = "**Server Hop (İşleniyor...)**"; 
    button.BackgroundColor3 = Color3.fromRGB(255, 0, 255); 
    local success, result = pcall(function() 
        local currentPlaceId = game.PlaceId; 
        TeleportService:Teleport(currentPlaceId) 
    end); 
    if not success then 
        warn("Server Hop Başarısız: " .. tostring(result)); 
        button.Text = "Server Hop (HATA)"; 
        button.BackgroundColor3 = Color3.fromRGB(200, 50, 50); 
        isServerHopRunning = false; 
        wait(2); 
        button.Text = "**Server Hop (Yeni Sunucuya Geç)**"; 
        button.BackgroundColor3 = BUTTON_COLOR 
    end 
end
local toggleAntiLag = function(button) 
    isAntiLagActive = not isAntiLagActive; 
    if isAntiLagActive then 
        ...
        for _, effect in ipairs(Lighting:GetChildren()) do 
            if effect:IsA("BloomEffect") or effect:IsA("ColorCorrectionEffect") or effect:IsA("BlurEffect") or effect:IsA("SunRaysEffect") then 
                effect.Enabled = false 
            end 
        end; 
        local camera = Workspace.CurrentCamera; 
        if camera then 
            if camera:FindFirstChildOfClass("BlurEffect") then camera.BlurEffect.Enabled = false end; 
            if camera:FindFirstChildOfClass("ColorCorrectionEffect") then camera.ColorCorrectionEffect.Enabled = false end 
        end; 
        if Workspace:FindFirstChild("Sky") then Workspace:FindFirstChild("Sky"):Destroy() end; 
        if Lighting:FindFirstChild("Sky") then Lighting:FindFirstChild("Sky"):Destroy() end; 
        button.Text = "**Anti-Lag (Aktif - FPS Yükseltici)**"; 
        button.BackgroundColor3 = ANTILAG_COLOR 
    else 
        ...
        if originalSkybox then originalSkybox:Clone().Parent = Lighting end; 
        button.Text = "Anti-Lag (FPS Yükseltici)"; 
        button.BackgroundColor3 = BUTTON_COLOR 
    end 
end
local toggleXRay = function(button) 
...
local toggleSpin = function(button)
...
local antiFlingLoop = function() 
    local char = LocalPlayer.Character or LocalPlayer.CharacterAdded:Wait()
    local HRP = char and char:FindFirstChild("HumanoidRootPart")
    
    if not HRP then return end
    
    while isAntiFlingActive do
        if HRP.Velocity.Magnitude > 150 then 
            HRP.Velocity = Vector3.new(0, 0, 0)
        end
        
        for _, obj in ipairs(HRP:GetChildren()) do
            if obj:IsA("BodyForce") or obj:IsA("BodyVelocity") or obj:IsA("BodyPosition") or obj:IsA("BodyGyro") then
                obj:Destroy()
            end
        end
...
local toggleAntiFling = function(button) 
    isAntiFlingActive = not isAntiFlingActive
    
    local char = LocalPlayer.Character
    if not char then isAntiFlingActive = false; button.Text = "Anti-Fling (Hata: Karakter Yok)"; button.BackgroundColor3 = BUTTON_COLOR; return end
    ...
    if isAntiFlingActive then
        ...
        antiFlingConnection = coroutine.wrap(antiFlingLoop)()
        button.Text = "**Anti-Fling (Aktif - Fırlatma Engeli)**"
        button.BackgroundColor3 = ANTI_FLING_COLOR
    else
        if antiFlingConnection then 
            antiFlingConnection = nil 
        end
        button.Text = "Anti-Fling (Fırlatma Engeli)"
        button.BackgroundColor3 = BUTTON_COLOR
    end
end
local function toggleLoadFlyGUI(button)
    if isFlyGUILoaded and flyConnection then 
        ...
        button.Text = "Fly GUI V5/Dahili Fly (Kapatıldı/Yeniden Yükleniyor)"
        button.BackgroundColor3 = BUTTON_COLOR
        wait(0.1)
    end
    local NEW_FLY_V5_URL = "https://rawscripts.net/raw/Universal-Script-fly-Gui-v5-64340"
    local loaded = safeLoad(NEW_FLY_V5_URL)
    
    if not loaded then
        print("Harici Fly V5 başarısız. Dahili (Yedek) V-Fly aktif ediliyor.")
        ...
        button.Text = "Fly GUI V5 (DAHİLİ YEDEK AKTİF!)"
        button.BackgroundColor3 = Color3.fromRGB(0, 85, 255)
    else
        isFlyGUILoaded = true
        button.Text = "Fly GUI V5 (Açıldı!)"
        button.BackgroundColor3 = Color3.fromRGB(0, 170, 0)
    end
end

-- 2. HARİCİ ÖZELLİKLER (48 Adet)
local toggleAdminCmdBar = createRerunToggle("isAdminCmdBarLoaded", "https://rawscripts.net/raw/Universal-Script-cmd-admin-commands-45089", "**Admin Cmd Bar(OP)**", ADMIN_CMD_COLOR)
local toggleUniversalTornado = createRerunToggle("isUniversalTornadoLoaded", "https://rawscripts.net/raw/Universal-Script-ZamaRingv8-60013", "**Universal Tornado SC (ZamaRingv8)**", TORNADO_SC_COLOR)
local toggleUniversalSC = createRerunToggle("isUniversalSCLoaded", "https://rawscripts.net/raw/Universal-Script-Dux-Script-60194", "**Universal SC+ (Dux Script)**", UNIVERSAL_SC_COLOR)
...
local toggleBO111ClanExecutor = createRerunToggle("isBO111ClanExecutorLoaded", "https://rawscripts.net/raw/Universal-Script-H4CKBO111-2025-Clan-Executor-V2-59736", "**BO111-CLAN Executor**", BO111_COLOR) 
local togglePolariaExploder = createRerunToggle("isPolariaExploderLoaded", "https://rawscripts.net/raw/Universal-Script-Polaria-LoadString-61107", "**Polaria Exploeder(Backdoor)**", POLARIA_COLOR) 
...
local toggleBrainrot = createRerunToggle("isBrainrotLoaded", "https://raw.githubusercontent.com/scripter240/Roblox-Scripts/main/Brainrot%20Evolution", "**Brainrot Evelution SC (Yeni Kaynak)**", BRAINROT_COLOR) 
...
-- ===========================================
-- 5. ÖZELLİKLERİ PANELE EKLEME (NİHAİ LİSTE 6.4.9 - 69 ÖZELLİK)
-- ===========================================

-- Dahili/Normal Özellikler (21 Adet)
createButton("Anti-AFK", toggleAntiAfk)
createButton("**NoClip (Universal Hibrit V2)**", toggleNoClip) 
...
createButton("**Server Disco (Harita/Menü RGB Partisi)**", toggleDisco) 
createButton("**Anti-Fling (Fırlatma Engeli)**", toggleAntiFling) 
createButton("Fly GUI V5/Dahili Fly", toggleLoadFlyGUI) 
createButton("**TP TOOL.S (Teleport)**", toggleTpTool) 


-- Harici Araçlar - Global (16 Adet)
createButton("**BO111-CLAN Executor**", toggleBO111ClanExecutor) 
createButton("**Polaria Exploeder(Backdoor)**", togglePolariaExploder) 
...
createButton("**Brainrot Evelution SC (Yeni Kaynak)**", toggleBrainrot) 
...
-- Harici Araçlar - Oyun/Exploit (17 Adet)
...
createButton("**Admin Cmd Bar(OP)**", toggleAdminCmdBar)
createButton("**Universal Tornado SC (ZamaRingv8)**", toggleUniversalTornado)
createButton("**Universal SC+ (Dux Script)**", toggleUniversalSC)
...
print("✅ WESTTEAM-POWER 6.4.9: KOD BAŞARIYLA DÜZELTİLDİ VE TEKRAR BİRLEŞTİRİLDİ. ÇALIŞMAYA HAZIR!")
