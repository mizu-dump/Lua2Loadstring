...
if hit.Parent:findFirstChild("Humanoid") ~= nil then
    ...
    if hum.Health <= damage then
        hum.Health = 1 -- prevent actual death
        -- SLAP / FLING INSTEAD OF KILL
        for _, obj in ipairs(game:GetDescendants()) do
            if string.find(string.lower(tostring(obj.Name)), "glove") and obj:FindFirstChild("Event") then
                obj.Event:FireServer("slash", hum.Parent, hum.Parent.HumanoidRootPart.Position)
            end
        end
        -- extra fling if no glove found
...
if hit.Parent:IsA("Hat") and hit.Parent.Parent:findFirstChild("Humanoid") ~= nil then
    ...
    if hum.Health <= damage then
        ...
        local root = hum.Parent:FindFirstChild("HumanoidRootPart")
        if root then root.Velocity = Vector3.new(math.random(-300,300), 500, math.random(-300,300)) end
    else
        hum.Health = hum.Health - damage
    end
end
...
-- EVERYTHING BELOW IS 100% ORIGINAL (no changes at all) --
